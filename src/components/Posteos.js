...
import Informacion from "./data.json";
...
export default function Posteos({funcion}) {
  return (
    <div className='container'>
      {Informacion.map(x=>{
        return <Post key={x.id} titulo={x.titulo} texto={x.texto} likes={x.likes} metodo={funcion}/>
      })}
    </div>
  )
}
